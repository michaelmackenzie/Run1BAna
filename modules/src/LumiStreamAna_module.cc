// ======================================================================
//
// LumiStreamAna_module.cc : Analyze lumi stream info
//
// ======================================================================
...
namespace mu2e {
  ...
  class LumiStreamAna : public art::EDAnalyzer {
  ...
  public:
    ...
    explicit LumiStreamAna(const art::EDAnalyzer::Table<Config>& config);
    virtual ~LumiStreamAna() {}
  ...
  LumiStreamAna::LumiStreamAna(const art::EDAnalyzer::Table<Config>& config) : art::EDAnalyzer{config}
  ...
  //--------------------------------------------------------------------------------
  void LumiStreamAna::beginJob() {
    art::ServiceHandle<art::TFileService> tfs;
    art::TFileDirectory infoDir = tfs->mkdir("info");

    _hNCaloHits     = infoDir.make<TH1F>("hNCaloHits", "N(Calorimeter hits);N(hits);Events", 100, 0., 5000.);
    ...
    _hNPOT_CaloEnergy = infoDir.make<TH1F>("hNPOT_CaloEnergy", "N(POT) from Calorimeter energy;N(POT);Events", 100, 0., 2.e8);

  }

  void LumiStreamAna::endJob() {
    std::cout << "LumiStreamAna::" << __func__ << ": Read information for an accumulated " << _nevents << " events\n";
  }

  //--------------------------------------------------------------------------------
  void LumiStreamAna::fillCalo(std::vector<art::Handle<mu2e::IntensityInfosCalo>>& handles) {
    if(_diagLevel > 0) {
      std::cout << "LumiStreamAna::" << __func__ << ": Processing " << handles.size() << " handles"
                << std::endl;
    }
    for (const auto& handle : handles) {
      if (!handle.isValid() || handle->empty()) {
        if(_diagLevel > 0) {
          std::cout << "LumiStreamAna::" << __func__ << ": Invalid or empty calo info handle!"
      ...
      for(auto& info : *handle) {
        ...
        // estimate N(POT)
        const auto npot_caloEnergy = 0.; // RecoNPOT::POTfromCaloEnergy(info.caloEnergy());
        _hNPOT_CaloEnergy->Fill(npot_caloEnergy);
      }
    }
  }

  //--------------------------------------------------------------------------------
  void LumiStreamAna::fillTrackerHits(std::vector<art::Handle<mu2e::IntensityInfosTrackerHits>>& handles) {
    if(_diagLevel > 0) {
      std::cout << "LumiStreamAna::" << __func__ << ": Processing " << handles.size() << " handles"
                << std::endl;
    }
    for (const auto& handle : handles) {
      if (!handle.isValid() || handle->empty()) {
        if(_diagLevel > 0) {
          std::cout << "LumiStreamAna::" << __func__ << ": Invalid or empty tracker hit info handle!"
                    << " Valid = " << handle.isValid() << " and size = " << ((handle.isValid()) ? ((int) handle->size()) : -1)
                    << std::endl;
        }
        continue;
  ...
  //--------------------------------------------------------------------------------
  void LumiStreamAna::fillTimeClusters(std::vector<art::Handle<mu2e::IntensityInfosTimeCluster>>& handles) {
    if(_diagLevel > 0) {
      std::cout << "LumiStreamAna::" << __func__ << ": Processing " << handles.size() << " handles"
                << std::endl;
    }
    for (const auto& handle : handles) {
      if (!handle.isValid() || handle->empty()) {
        if(_diagLevel > 0) {
          std::cout << "LumiStreamAna::" << __func__ << ": Invalid or empty time clusters info handle!"
                    << " Valid = " << handle.isValid() << " and size = " << ((handle.isValid()) ? ((int) handle->size()) : -1)
                    << std::endl;
        }
        continue;
  ...
  //--------------------------------------------------------------------------------
  void LumiStreamAna::fillEventHeaders(std::vector<art::Handle<mu2e::EventHeaders>>& handles) {
    if(_diagLevel > 0) {
      std::cout << "LumiStreamAna::" << __func__ << ": Processing " << handles.size() << " handles"
                << std::endl;
    }
    for (const auto& handle : handles) {
      if (!handle.isValid() || handle->empty()) {
        if(_diagLevel > 0) {
          std::cout << "LumiStreamAna::" << __func__ << ": Invalid or empty event headers handle!"
                    << " Valid = " << handle.isValid() << " and size = " << ((handle.isValid()) ? ((int) handle->size()) : -1)
                    << std::endl;
        }
        continue;
  ...
  //--------------------------------------------------------------------------------
  void LumiStreamAna::endSubRun(const art::SubRun& sr) {
    ...
    if(_diagLevel > 0) {
      std::cout << "LumiStreamAna::" << __func__ << ": Subrun "
  ...
  //--------------------------------------------------------------------------------
  void LumiStreamAna::analyze(const art::Event& event) {
    // for printout use
...
DEFINE_ART_MODULE(mu2e::LumiStreamAna)
