...
//------------------------------------------------------------------------------
void plotNeutronvsBkgFromNtuple(const char* tag = "v40", TString hist_tag = "",
  ...
  const vector<TString> included_dataset_keys = {"neut", "mnbs", "csms", "prot", "fgam", "pgam"};
  map<TString, TFile*> files;
  if(!openIncludedDatasetFiles(datasets, included_dataset_keys, files, hist_tag, __func__)) return;

  TFile* f_sig = getDatasetFile(files, "neut");
  TFile* f_bkg = getDatasetFile(files, "mnbs");
  if(!f_sig || !f_bkg) {
    Error(__func__, "Missing required neutron or background datasets in included keys");
    return;
  }
  ...
  const vector<TString> enabled_process_ids = {
    "neutrons", "neutrons_pu", "neutrons_cpu", "cosmics", "protons", "poly", "rmc", "pileup_lo", "pileup_ot", "calomu"
  };
  ...
  norm_sig_ = getNorm(datasets["neut"], f_sig, npot, livetime_week_);
  ...
  for(auto& process : processes_) {
    ...
    process.is_signal = process.name.Contains("Neutron");
  ...
  dir_ = (tag) ? Form("figures/neutron_vs_bkg_nt_%s", tag) : "figures/neutron_vs_bkg_nt";
  ...
  for(const int set : proc_sets) {
    for(const bool normalize : {false}) {
      ...
      plot("time_cluster_avg_edep"          , set, normalize, 1,   0., 0.005);
      plot("prot_time_cluster_avg_edep"     , set, normalize, 1,   0., 0.005);
