...
void plotRMCvsBkgFromNtuple(const char* filename_sig = "Run1BAna.fgam4b1s51r0000.hist",
  ...
  // Plot the histograms
  vector<int> sets = {0, 2, 4, 5, 10, 11, 15, 16, 20, 22, 25, 30, 31, 35, 36};
  for(const int set : sets) {
    plot_gen_eff(f_sig, set);
    for(const bool normalize : {false, true}) {
      plot("energy"                 , set, normalize, 1,  70.,  120., f_sig, f_bkg);
      plot("time"                   , set, normalize, 2, 400., 2000., f_sig, f_bkg);
      plot("radius"                 , set, normalize, 1, 300.,  700., f_sig, f_bkg);
      plot("disk"                   , set, normalize, 1,   0.,    2., f_sig, f_bkg);
      plot("frac_first_crystal"     , set, normalize, 1,   1.,   -1., f_sig, f_bkg);
      plot("frac_first_two_crystals", set, normalize, 1,   1.,   -1., f_sig, f_bkg);
      plot("ncr"                    , set, normalize, 1,   0.,   10., f_sig, f_bkg);
      plot("second_moment"          , set, normalize, 5,   1.,   -1., f_sig, f_bkg);
      plot("t_var"                  , set, normalize, 1,   0.,    5., f_sig, f_bkg);
      plot("time_cluster_dt"        , set, normalize, 1, -50.,   50., f_sig, f_bkg);
    }
  }
