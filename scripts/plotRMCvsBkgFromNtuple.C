...
void plotRMCvsBkgFromNtuple(const char* filename_sig = "Run1BAna.fgam4b1s51r0000.hist",
  ...
  std::cout << "N(sampled): RMC = " << nnt_sig_ << " Bkg = " << nnt_bkg_ << " DIO = " << nnt_dio << std::endl;
  std::cout << "N(saved): RMC = " << n_saved_sig << " Bkg = " << n_saved_bkg << " DIO = " << n_saved_dio << std::endl;
  std::cout << "Eff(dataset): RMC = " << sig_skim_eff_ << " Bkg = " << norm_b << " DIO = " << dio_skim_eff << std::endl;
  std::cout << "N(POT) = " << npot << " N(muons) = " << nmuons << " N(RMC) = " << nrmc << " N(DIO) = " << ndio << std::endl;
  std::cout << "Norms: RMC = " << norm_sig_ << " Bkg = " << norm_bkg_ << " DIO = " << norm_dio << std::endl;
  std::cout << "N(RMC | 0) = " << getNSampled(f_sig, 0)*norm_sig_ << " N(Bkg | 0) = " << getNSampled(f_bkg, 0)*norm_bkg_ << std::endl;
  std::cout << "I(RMC | 0) = " << getIntegral(f_sig, 0)*norm_sig_ << " I(Bkg | 0) = " << getIntegral(f_bkg, 0)*norm_bkg_ << std::endl;
  ...
  for(const int set : proc_sets) {
    for(const bool normalize : {false, true}) {
      plot("cluster_energy"                 , set, normalize, 1,  70.,  120.);
  ...
  // Plot the histograms
  vector<int> sets = {0, 1, 3, 4, 18, 19, 20, 23, 24, 30, 32, 35, 37, 38};
